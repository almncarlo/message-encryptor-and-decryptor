...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        '''
        # initializes cipher dictionary
        cipher_dict = {}
        # iterates through length of given vowel combination
        for i in range(len(vowels_permutation)):
            # add char from vowel input as key with its value
            # as its index counterpart in 'aeiou'
            cipher_dict.update({vowels_permutation[i].upper():VOWELS_UPPER[i]})
            cipher_dict.update({vowels_permutation[i].lower():VOWELS_LOWER[i]})
        for i in range(len(CONSONANTS_LOWER)):
            # consonants are equal to themselves and will not change
            cipher_dict.update({CONSONANTS_LOWER[i]:CONSONANTS_LOWER[i]})
            cipher_dict.update({CONSONANTS_UPPER[i]:CONSONANTS_UPPER[i]})
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for perm in get_permutations(VOWELS_LOWER):
            ...
            deciphered = self.apply_transpose(enc_dict)
            matched_word = []
            ...
            # keys: num of valid words
            # values: deciphered message
            cipher_shifts[sum(matched_word)] = deciphered
        # returns dictionary item with most valid words
        # a cipher_shifts key of more than 0 means that the item
